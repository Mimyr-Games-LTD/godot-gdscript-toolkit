...
def test_missing_cancellation_check_ok(code):
    simple_ok_check(code, disable=["unused-argument", "missing-ct-arg", "async-function-name", "missing-ct-param"])
...
def test_missing_cancellation_check_nok(code, line):
    simple_nok_check(code, "missing-ct-check", line=line, disable=["unused-argument", "missing-ct-arg", "async-function-name", "missing-ct-param"])
...
"""
func test1(ct):
    await method1(ct)  # gdlint: ignore=missing-ct-check
""",
"""
func test2(ct):
    # gdlint: ignore=missing-ct-check
    await method2(ct)
""",
"""
# gdlint: disable=missing-ct-check
func test3(ct):
    await method3(ct)

func test4(ct):
    await method4(ct)
# gdlint: enable=missing-ct-check
""",
])
